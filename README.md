...
### 🧮 Math Canvas
- **横屏全屏数学画布**：白板 + 工具栏 + 撤销 / 重做，**滚动 / 拖动 / 缩放 / 双击复位** 全部流畅
- **函数 / 几何 / 标注 / 预设** 一应俱全
...
├── CHANGELOG.md               # 详细更新日志
├── LICENSE                    # MIT
└── Knowledge_Learning_Repository_*.md   # 需求设计文档（不进入 git）
`
...
`ash
# 克隆
git clone https://github.com/wxuhao32/VocabHit.git
cd VocabHit
`
...
> ⚠️ 首次构建会下载 Gradle 8.9 + 依赖，请保持网络通畅。

### 方式三：本地 Web 预览（不打包 APK）
...
> 注意：本地预览**没有 WebView 桥接**（悬浮窗 / 文档解析 / OCR 不可用），仅用于 UI 调试。
